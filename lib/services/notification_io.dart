// lib/services/notification_io.dart
//
// File ini HANYA dikompilasi di platform non-web (Android, iOS, Desktop).
// Berisi semua logika notifikasi yang bergantung pada flutter_local_notifications.
// Dipanggil oleh notification_service.dart dengan conditional import internal.
...
/// Implementasi penuh NotificationService untuk Android/iOS.
class NotificationService {
    ...
    try {
      ...
      for (final entry in scheduled) {
        try {
          ...
          if (colonIndex == -1) { toRemove.add(entry); continue; }
          final todoId = int.parse(entry.substring(0, colonIndex));
          final reminderAt = DateTime.parse(entry.substring(colonIndex + 1));
          if (reminderAt.isBefore(DateTime.now())) { toRemove.add(entry); continue; }
          final tzDateTime = tz.TZDateTime.from(reminderAt, tz.local);
          await _scheduleWithFallback(todoId, 'Pengingat Todo', 'Todo perlu dikerjakan sekarang', tzDateTime);
  ...
  static Future<void> scheduleTodoReminder(Todo todo) async {
    ...
    if (todo.reminderAt.isBefore(now)) return;
  ...
  static Future<void> _scheduleWithFallback(int id, String title, String body, tz.TZDateTime scheduledTime) async {
    if (_plugin == null) return;
  ...
  static Future<void> showTestNotification() async {
    if (_plugin == null) return;
    try {
      await _plugin!.show(9999, 'Test Notification', 'Jika Anda melihat ini, notification system bekerja!', _details(), payload: 'test');
  ...
  static Future<void> scheduleTestNotificationWithDelay(int secondsDelay) async {
    try {
      ...
      await _scheduleWithFallback(9997, 'Debug: Test Scheduled Notif', 'Ini seharusnya muncul dalam $secondsDelay detik', tzDateTime);
